...
class Exception
{
	...
	public:
		...
		void what()   // error msg kya hai wo display krne ke lea hai
		{
			cout<<"\t\t"<< errNo<<" :: "<< msg<<endl;
		}
};
...
class Room
{
	...
	public:
		Room(string roomType,int noOfBeds,double rent,int roomNo )  //contructor
		{
		...
		void setRoom()   // fuddu hai use nahi kiya hai usne ye function kabhi
		{
		...
		void vacateRoom(int rno)   // saare room ko kahaali krne ke lea
		{
			int i;
			for(i=0;i<6;i++)
			{
		...
		void displayAvailable(Room r[6])
		{
			...
			for(i=0;i<6;i++)
			{
				if(r[i].status==0)  // jis ka satatus 0 hai
				r[i].displayDetail();
			}
		}
...
class Customer
{
	...
	public:
		...
		void setData()
		{
			...
			string dt = ctime(&now);
			this->checkInTime = dt;
			ofstream customer;   // file kholne ke lea
			...
			customer.close();   // band kr di file ko
...
class RoomCustomer : public Customer   //inheritance
{
	public:
		double rbill;
		Room r;
...
class Employee
{
	...
	public:
		...
		virtual void performDuty()=0;   // pyure virtual function or do nothing function
		virtual ~Employee(){
		}
};
...
class Hotel
{
	...
	public:
		...
		void generateID(Customer *c)    // customer id generate random
		{
			time_t t;
